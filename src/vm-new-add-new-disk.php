<?php
 ...
 if ($_SESSION['admin-login']) {
  ...
  if (@$lv->connect($GLOBALS['address'])) {
   $disk_template = file_get_contents($GLOBALS['template-path'] . '/vm-new-add-new-disk.html');
   $storage_template = file_get_contents($GLOBALS['template-path'] . '/vm-new-add-new-disk-row.html');
   $storages_html = '';
